...
def test_p2wpkh_payjoin(client):
    ...
    details = messages.SignTx(lock_time=1516634)

    # Expected responses when there is a sender fee bump.
    responses_with_fee_confirmation = [
        request_input(0),
    ...
    # Expected responses when there is no sender fee bump.
    responses_without_confirmation = [
        resp
        for resp in responses_with_fee_confirmation
        if not isinstance(resp, messages.ButtonRequest)
    ]

    # Scenario 1: No fee bump by sender or receiver.
    with client:
        client.set_expected_responses(responses_without_confirmation)
        _, serialized_tx = btc.sign_tx(
            client,
            "Testnet",
            [inp1, inp2],
    ...
    with client:
        client.set_expected_responses(responses_with_fee_confirmation)
        _, serialized_tx = btc.sign_tx(
            client,
            "Testnet",
            [inp1, inp2],
...
def test_p2wpkh_in_p2sh_remove_change(client):
    # Test fee bump with change-output removal.
    # TODO: Actually remove the change-output.
    ...
    out2 = messages.TxOutputType(
        address_n=parse_path("49h/1h/0h/1/3"),
        amount=1,  # originally fee was 3780, now 98060 - 1. TODO: Actually remove.
    ...
    with client:
        client.set_expected_responses(
            [
                ...
                request_input(0, TXHASH_334cd7),
                request_input(1),
                request_input(1, TXHASH_334cd7),
                request_output(0),
                request_output(0, TXHASH_334cd7),
                request_output(1),
                request_output(1, TXHASH_334cd7),
                messages.ButtonRequest(code=B.SignTx),
                request_input(0),
